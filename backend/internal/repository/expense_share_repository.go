...
// SumPaidByGroup 统计群组内各成员付款总额（付款人维度；传 tx 时复用该事务）。
func (r *ExpenseShareRepository) SumPaidByGroup(tx *gorm.DB, groupID uint) (map[uint]float64, error) {
	type row struct {
		UserID uint
		Total  float64
	}
	var rows []row
	if err := txOrDB(tx, r.db).Model(&model.Expense{}).
...
// SumOwedByGroup 统计群组内各成员应付总额（参与人维度，仅有效消费；传 tx 时复用该事务）。
func (r *ExpenseShareRepository) SumOwedByGroup(tx *gorm.DB, groupID uint) (map[uint]float64, error) {
	type row struct {
		UserID uint
		Total  float64
	}
	var rows []row
	if err := txOrDB(tx, r.db).Table("expense_shares AS s").
...
// MarkSettledByExpenses 将某批消费的分摊明细标记为已结算（事务中调用）。
func (r *ExpenseShareRepository) MarkSettledByExpenses(tx *gorm.DB, expenseIDs []uint) error {
	if len(expenseIDs) == 0 {
		return nil
	}
	if err := txOrDB(tx, r.db).Model(&model.ExpenseShare{}).
